               ...
               failure:(NetRequestFailedBlock)fail
{
    ...
    if (isPhp) {
        ...
        if ([SQRDataSave takeOutDataFromDataEnum:SaveDataEnum_Token customKey:nil]) {
            ...
            // 1 用户端  2 商家端  3 企业购用户端 4 企业购商家端  5 三合一商家端
            [manager.requestSerializer setValue:@"5" forHTTPHeaderField:@"app"];
        }
    }
 ...
 *  @param fail       失败Block
 */
- (void)postUploadVideoWithUrl:(NSString *)urlString
                         data:(NSData *)data
                    parameters:(id)parameters
                     dataImage:(NSString *)dataImage
                      progress:(NetRequestProgressBlock)progress
    ...
    [manager POST:urlString parameters:parameters headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        [formData appendPartWithFileData:data name:dataImage ? dataImage : @"uploadPic" fileName:fileName mimeType:@"video/mp4"];
